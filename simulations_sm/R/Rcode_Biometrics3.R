...
onestage_under_hal <- function(dat, rate = 0.8,nfolds=5, ss=2,trunc=0.05){
  ...
  for(i in 1:nfolds){
    ...
    train <- dat[-testIndexes,]
     mod_cv =SuperLearner(Y =  train$A, X = train[,-c(1:4)], family = binomial(),   SL.library = c("SL.randomForest"),method = "method.NNloglik")
      preds_rf[testIndexes] = as.vector(predict(mod_cv, test[,-c(1:4)], onlySL = T)$pred)
...
#  preds_rf[preds_rf < 0.02] = 0.02
  ...
  for(i in 1:nfolds){
    ...
    test <- dat[testIndexes,]
    train <- dat[-testIndexes,]
   # Ymod_cv = tryCatch(fit_hal(X = train[,c("A","x1","x2")], Y = train$Y, yolo = FALSE, max_degree = 2), error=function(e) e, warning=function(w) w)
   # Ymod_cv = randomForest(x = train[,c("A","x1","x2")], y = train$Y)
   Ymod_cv = SuperLearner(Y =  train$Y, X = train[,c("A","x1","x2")], SL.library = c("SL.randomForest"))
 ...
 #   Ymod_cv = SuperLearner(Y =  train$Y, X = train[,c("A","x1","x2")], SL.library = c("SL.randomForest"))
    ntest<-test[,c("A","x1","x2")]
    ntest$A<-1
    Q1[testIndexes] = as.vector(predict(Ymod_cv,  ntest, onlySL = T)$pred)
   # Q1[testIndexes] = predict(Ymod_cv, new_data = ntest)
    
  #  ntest$A<-0
  #  Q0[testIndexes] = predict(Ymod_cv, new_data = ntest)
...
##  #Q0<-dat$Q0
  ...
  cp_dr_dmiss = mean(dr.est.dmiss - 1.96*sigma.star.dmiss <= true & dr.est.dmiss + 1.96*sigma.star.dmiss >= true)
  cp_dr_ymiss = mean(dr.est.ymiss - 1.96*sigma.star.ymiss <= true & dr.est.ymiss + 1.96*sigma.star.ymiss >= true)
  cp_lr = mean(lr_ipw - 1.96*lr_se <= true & lr_ipw + 1.96*lr_se >= true)
  
  
  dr_est_vec<-c(dr.est,dr.est.dmiss,dr.est.ymiss)
  names(dr_est_vec)<-c("dr","dr_dmiss","dr_ymiss")
  bias<-( dr_est_vec-true)
  rnbias<-sqrt(n) * ( bias)
  names(rnbias)<-c("rnbias_dr","rnbias_dr_dmiss","rnbias_dr_ymiss")
...
names(sigma.star.all)<-c("sigma.star","sigma.star.dmiss","sigma.star.ymiss")
  ...
  names(rnmse)<-c("rnmse_dr","rnmse_dr_dmiss","rnmse_dr_ymiss")
  rndcar_dr<-sqrt(n)*D_CAR_dr
  names(rndcar_dr)<-"rndcar_dr"
  
  cp.drs<-c(cp_dr,cp_dr_dmiss,cp_dr_ymiss)
  names(cp.drs)<-c("cp_dr","cp_dr_dmiss","cp_dr_ymiss")
