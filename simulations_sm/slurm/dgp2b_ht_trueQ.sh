#!/bin/bash
# Job name:
#SBATCH --job-name=ipwuhal_dgp2b_ht_nsIPW_Q0
#
# Working directory:
...
R CMD BATCH --no-save --no-restore \
  '--args dgp=2b Mncv_max=80 trunc_max=0.4 trunc_len=5 ipw=ht Q_reg=Q0 g_trunc=profile' \
  R/02_est_npipw_hal.R logs/dgp2b_ht_Q0_profile.Rout
R CMD BATCH --no-save --no-restore \
  '--args dgp=2b Mncv_max=80 trunc_max=0.4 trunc_len=5 ipw=ht Q_reg=Q0 g_trunc=joint' \
  R/02_est_npipw_hal.R logs/dgp2b_ht_Q0_joint.Rout
