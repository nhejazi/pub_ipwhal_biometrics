context("Check stabilized IPW estimator using truth and NP-MLE")
...
test_that("Stabilized IPW is AL with known IC for true propensity score", {
  expect_equal(est_diff, true_ic, tol = 1e-3)
})
...
test_that("Stabilized IPW is AL with (IC-D_CAR) for NP-MLE propensity score", {
  expect_equal(est_diff, est_ic, tol = 1e-3)
})
